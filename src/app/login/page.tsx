...
function LoginForm() {
  ...
  return (
    <div className="w-full max-w-md space-y-7 relative z-10 glass p-10 rounded-[32px]">
      <div className="text-center space-y-2">
        ...
        <p className="text-zinc-500 font-medium">Log in to continue watching.</p>
      </div>

      {justRegistered && (
        <div className="px-4 py-3 bg-green-500/10 border border-green-500/30 rounded-xl">
          <p className="text-green-400 text-sm font-medium">✓ Account created! Sign in below.</p>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <button onClick={handleSubmit} disabled={loading}
          className="w-full py-4 bg-white text-black font-black rounded-2xl hover:bg-zinc-200 transition-all transform active:scale-95 disabled:opacity-50">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button className="flex items-center justify-center gap-2.5 py-3 bg-zinc-950 border border-zinc-800 rounded-2xl hover:bg-zinc-900 transition-colors">
      ...
      <p className="text-center text-sm text-zinc-500">
        New to Anime Portal?{" "}
        <Link href="/signup" className="text-blue-500 font-bold hover:underline">Create an account</Link>
      </p>
    </div>
  );
}
